...
RET=`echo 'SELECT \`name\` FROM \`params\` WHERE name="clamavmail"' | mysql -h $dbhost $dbname -u $dbuser -p$dbpass --batch |wc -l`

if [ $RET -eq 0 ]; then
  echo "adding clamav admin param"
  mysql -h $dbhost $dbname -u $dbuser -p$dbpass < /var/cache/se3_install/se3-clamav/clamav-mail-db.sql >/dev/null 2>&1
fi
